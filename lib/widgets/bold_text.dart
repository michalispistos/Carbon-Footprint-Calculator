...
class TextBold extends StatelessWidget{
  ...
  const TextBold({Key? key, required this.text, this.regex = r'\d+'}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Text.rich(TextSpan(
        children: parts.map((e) => TextSpan(
            text: e.text,
            style: (e.isBold)
                ? const TextStyle(fontWeight: FontWeight.bold, color: CupertinoColors.black)
                : const TextStyle(fontWeight: FontWeight.normal, color: CupertinoColors.black)))
            .toList()));
  }
